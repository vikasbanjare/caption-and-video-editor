/**
 * Stub transcriber — the zero-config default (WEBSAASPLAN.md §6 Phase 1). It
 * never touches the media; it returns a believable sample transcript spread
 * across the clip duration so the editor works end to end before any ASR key is
 * configured. Returns SRT (no real word timings), which the engine splits
 * evenly for preview.
 */
...
export function buildStubSrt(durationSeconds: number, language: string): string {
